...
class EntityExpression(object):
    ...
    * "{code:^([^_]+)}/{code:^[^_]+(.+)}" + {"code": "foo_bar"} ==> "foo/bar"
    ...
    def generate_name(self, values):
        ...
        logger.info("🧪 [DEBUG] generate_name() 호출됨")
        logger.info(f"📦 entity(values): {values}")
        logger.info(f"🧩 필드 표현식(self._field_name_expr): {self._field_name_expr}")
        logger.info(f"🌀 시도할 표현식 리스트: {self._sorted_exprs}")
        ...
        for expr in self._sorted_exprs:
            ...
            print(f"🧪 시도한 expression: {expr}, 결과: {val}")
            if val is not None:
        ...
        if val is None:
            ...
            print("❌ generate_name 실패 - 아래 필드가 비어 있을 가능성 있음:")
            print(f"📦 entity values: {values}")
            print(f"🔍 필드 표현식: {self._field_name_expr}")
            print(f"🔁 전체 표현식 리스트: {self._sorted_exprs}")
